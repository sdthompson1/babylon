        ...
        for (unsigned int i = 0; i < sizeof(funs)/sizeof(funs[0]); ++i) {
            for (unsigned int j = 0; j < sizeof(sizes)/sizeof(sizes[0]); ++j) {
                for (int s = 0; s < 2; ++s) {
                    ...
                    add_to_type_env(type_env, &buf[1], fun_ty, true, true, false, false, ALLOC_UNKNOWN);
                    add_to_type_env(expanded_type_env, &buf[1], copy_type(fun_ty), true, true, false, false, ALLOC_UNKNOWN);
