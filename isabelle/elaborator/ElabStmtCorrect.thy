...
(* In NotGhost mode the return type of a well-typed impure call is complete: the
   ...
   type arguments are complete, so the instantiated return type is too. *)
lemma core_impure_call_type_notghost_complete:
  assumes ct: "core_impure_call_type env NotGhost fnName tyArgs tmArgs = Some retTy"
      and wf: "tyenv_well_formed env"
  shows "is_complete_type retTy"
proof -
  from core_impure_call_type_fn_facts[OF ct] obtain funInfo where
    ...
    ret_eq: "retTy = apply_subst (fmap_of_list (zip (FI_TyArgs funInfo) tyArgs))
                                 (FI_ReturnType funInfo)"
    by blast
  have fg: "FI_Ghost funInfo = NotGhost" using ng by (cases "FI_Ghost funInfo") auto
  have "tyenv_fun_return_types_complete env"
    using wf unfolding tyenv_well_formed_def by blast
  hence ret_cp: "is_complete_type (FI_ReturnType funInfo)"
    using fi fg unfolding tyenv_fun_return_types_complete_def by blast
  show ?thesis
    unfolding ret_eq
  ...
  "coerce_term_to_type env loc tm srcTy tgtTy =
    (case unify_upto_coercion (\<lambda>n. n |\<notin>| TE_TypeVars env) srcTy tgtTy of
       Some subst \<Rightarrow>
         ...
         else Inr (insert_cast (apply_subst subst srcTy) (apply_subst subst tgtTy)
...
proof -
  ...
  have tgtTy_wkD: "is_well_kinded ?envD tgtTy"
    using is_well_kinded_extend_env_with_tyvars[OF wk] .
  have tgtTy_rtD: "ghost = NotGhost \<longrightarrow> is_runtime_type ?envD tgtTy"
    using rt is_runtime_type_extend_env_with_tyvars by blast
  ...
  from coerce obtain subst where
    ...
    subst_complete: "typesubst_complete subst" and
  ...
  have subst_cp: "\<forall>ty' \<in> fmran' subst. is_complete_type ty'"
    using subst_complete unfolding typesubst_complete_def .
...
proof -
  ...
  have tgtTy_wkE: "is_well_kinded ?envE tgtTy"
    using is_well_kinded_extend_env_with_tyvars[OF wk] .
  have tgtTy_rtE: "ghost = NotGhost \<longrightarrow> is_runtime_type ?envE tgtTy"
    using rt is_runtime_type_extend_env_with_tyvars by blast
  ...
  from rcr obtain subst where
    ...
    by (auto simp: reconcile_call_result_def Let_def split: option.splits if_splits)
  have subst_cp: "\<forall>ty' \<in> fmran' subst. is_complete_type ty'"
    using subst_complete unfolding typesubst_complete_def .
  have subst_wk: "\<forall>ty' \<in> fmran' subst. is_well_kinded ?envE ty'"
  ...
  have ctE': "core_impure_call_type ?envE ghost fnName tyArgs' argTms'
    ...
    using apply_subst_core_impure_call_type
            [OF ctE wfE subst_wk subst_rt locals_unaffected ret_unaffected abs_no_subst subst_cp]
  ...
  proof (cases "apply_subst subst retTy = tgtTy")
    case True
    thus ?thesis using castOpt_eq tgt_id by (simp add: cast_result_type_def)
  next
    case False
    hence coer: "coercible (apply_subst subst retTy) tgtTy" using ok by simp
    have "castOpt = Some tgtTy" using castOpt_eq tgt_id False by simp
    thus ?thesis using coercible_cast_ok[OF coer wk] rt by (simp add: cast_result_type_def)
  qed
...
(* elab_vardecl_impure emits a CoreStmt_VarDeclCall that typechecks in env.
   Two flavors: inferred (no annotation, varTy = call return type, no cast), and
   annotated (varTy = annotation; reconcile_call_result applies the unifying
   substitution to the args and chooses the cast). *)
...
proof -
  ...
  proof
    assume ng: "ghost = NotGhost"
    have wfE: "tyenv_well_formed (extend_env_with_tyvars ?bodyEnv ghost next_mv next_mv')"
      using wfB tyenv_well_formed_extend_env_with_tyvars by blast
    show "is_complete_type retTy"
      using core_impure_call_type_notghost_complete[OF ctE[unfolded ng] wfE[unfolded ng]] .
  qed
  ...
  show ?thesis using wkrt ct retTy_cp
...
(* The impure-return helper advances the counter via elab_impure_call_term. *)
lemma elab_return_impure_next_mv:
  "elab_return_impure env elabEnv ghost loc tm next_mv = Inr (coreStmt, env', next_mv')
     \<Longrightarrow> next_mv \<le> next_mv'"
  by (auto simp: elab_return_impure_def Let_def
           dest!: elab_impure_call_term_next_mv
           split: BabTerm.splits sum.splits prod.splits if_splits)

(* The impure-return helper leaves the env unchanged (the temporary's binding is
   confined to the emitted block). *)
lemma elab_return_impure_env:
  "elab_return_impure env elabEnv ghost loc tm next_mv = Inr (coreStmt, env', next_mv')
     \<Longrightarrow> env' = env"
  by (auto simp: elab_return_impure_def Let_def
           split: BabTerm.splits sum.splits prod.splits if_splits)

(* elab_return_impure emits a CoreStmt_Block containing a CoreStmt_VarDeclCall that
   binds the call result to the return@@tmp temporary, declared at the function's
   return type (reconcile_call_result choosing the cast, exactly as in the annotated
   branch of elab_vardecl_impure_correct), followed by a CoreStmt_Return of the
   temporary. The call is elaborated under bodyEnv = env with
   TE_ProofTopLevel := False, matching the Block rule's body env (as in
   elab_call_statement_correct). The Return of the temporary then typechecks because
   its declared type IS the return type, and ghost = TE_FunctionGhost env (an entry
   invariant of the elaborator's Return case, passed in as an assumption here). *)
lemma elab_return_impure_correct:
  assumes elab: "elab_return_impure env elabEnv ghost loc tm next_mv
                   = Inr (coreStmt, env', next_mv')"
    and gh: "ghost = TE_FunctionGhost env"
    and wf: "tyenv_well_formed env"
    and ee_wf: "elabenv_well_formed env elabEnv"
    and bound: "\<forall>n. n |\<in>| TE_TypeVars env \<longrightarrow> tyvar_fresh_ok n next_mv"
  shows "core_statement_type env ghost coreStmt = Some env'"
proof -
  let ?bodyEnv = "env \<lparr> TE_ProofTopLevel := False \<rparr>"
  let ?envE = "extend_env_with_tyvars ?bodyEnv ghost next_mv next_mv'"
  let ?retTy = "TE_ReturnType env"
  \<comment> \<open>Peel the elaborator's case chain: the value is a call, and the embedded call
      elaborates and reconciles against the return type successfully.\<close>
  from elab obtain rloc callee rargs where tm_eq: "tm = BabTm_Call rloc callee rargs"
    by (cases tm) (auto simp: elab_return_impure_def)
  from elab tm_eq obtain fnName finalTyArgs finalArgTms retTy where
    ec: "elab_impure_call_term ?bodyEnv elabEnv ghost False rloc callee rargs next_mv
           = Inr (fnName, finalTyArgs, finalArgTms, retTy, next_mv')"
    by (auto simp: elab_return_impure_def reconcile_call_result_def Let_def
             split: sum.splits prod.splits option.splits if_splits)
  \<comment> \<open>Reduce the elaborator equation by ec, leaving only the reconcile case
      (a plain simp rather than obtain-auto, which diverges on the stacked splits).\<close>
  have elab2: "(case reconcile_call_result env loc finalTyArgs finalArgTms retTy ?retTy of
                ...
                | Inr (castOpt, tyArgs', argTms') \<Rightarrow>
                    Inr (CoreStmt_Block
                         ...
                         env, next_mv'))
               = Inr (coreStmt, env', next_mv')"
    using elab unfolding tm_eq elab_return_impure_def Let_def by (simp add: ec)
  obtain castOpt tyArgs' argTms' where
    rcr: "reconcile_call_result env loc finalTyArgs finalArgTms retTy ?retTy
            = Inr (castOpt, tyArgs', argTms')"
  proof (cases "reconcile_call_result env loc finalTyArgs finalArgTms retTy ?retTy")
    case (Inl errs)
  ...
  have eeB: "elabenv_well_formed ?bodyEnv elabEnv"
    using ee_wf elabenv_well_formed_cong_env[where env' = ?bodyEnv and env = env] by simp
  have boundB: "\<forall>n. n |\<in>| TE_TypeVars ?bodyEnv \<longrightarrow> tyvar_fresh_ok n next_mv" using bound by simp
  \<comment> \<open>The call typechecks in bodyEnv's tyvar extension.\<close>
  have ctE: "core_impure_call_type ?envE ghost fnName finalTyArgs finalArgTms = Some retTy"
    using elab_impure_call_term_correct[OF ec wfB eeB boundB] .
  \<comment> \<open>The return type is well-kinded (and, in NotGhost mode, runtime - via gh and the
      tyenv_return_type_runtime clause) in bodyEnv, and is metavar-free.\<close>
  have retTy_wk: "is_well_kinded env ?retTy"
    using wf unfolding tyenv_well_formed_def tyenv_return_type_well_kinded_def by blast
  have wkB: "is_well_kinded ?bodyEnv ?retTy"
    using retTy_wk is_well_kinded_cong_env[of ?bodyEnv env] by simp
  have retTy_rt: "ghost = NotGhost \<longrightarrow> is_runtime_type env ?retTy"
    using wf gh unfolding tyenv_well_formed_def tyenv_return_type_runtime_def by auto
  have rtB: "ghost = NotGhost \<longrightarrow> is_runtime_type ?bodyEnv ?retTy"
    using retTy_rt is_runtime_type_cong_env[of ?bodyEnv env] by simp
  \<comment> \<open>In NotGhost mode the return type is complete (tyenv_return_type_complete, via gh).\<close>
  have retTy_cp: "ghost = NotGhost \<longrightarrow> is_complete_type ?retTy"
    using wf gh unfolding tyenv_well_formed_def tyenv_return_type_complete_def by auto
  \<comment> \<open>reconcile_call_result depends on the env only through TE_TypeVars, which
  ...
  \<comment> \<open>Assemble: the VarDeclCall types the temporary at ?retTy in bodyEnv; the Return
      ...
      function-ghost flag = ghost; the block discards the temporary's scope and
      returns env unchanged.\<close>
  show ?thesis using wkB rtB retTy_cp ct cast_ok gh
    by (cases ghost)
       (auto simp add: cs_eq env'_eq tyenv_lookup_var_def tyenv_var_ghost_def)
qed
...
next
  ...
  have subst_typed: "core_term_type ?envD Ghost (apply_subst_to_term subst coreInv)
    ...
    using apply_subst_to_term_preserves_typing
            [OF typed_ghost wfD subst_wk _ locals_unaffected ret_unaffected abs_no_subst
                unify_bool_range_complete[OF unif]] by simp
  ...
  have subst_typed: "core_term_type ?envC ghost (apply_subst_to_term subst coreCond)
    ...
    using apply_subst_to_term_preserves_typing
            [OF cond_typed wfC subst_wk subst_rt c_locals c_ret c_abs
                unify_bool_range_complete[OF unif]] .
...
proof (induction env elabEnv ghost stmt next_mv and env elabEnv ghost stmts next_mv
  ...
  proof (cases vorf)
    ...
    next
      case default thus ?thesis using "1.prems" Var by (auto split: sum.splits if_splits)
    next
      case init
      show ?thesis
...
next
  \<comment> \<open>Return: the void / no-value branches keep next_mv; the non-void value branch
      advances it only via the returned term's elab_term (pure) or via
      elab_return_impure's embedded call elaboration (impure).\<close>
  ...
  proof (cases vorf)
    ...
    next
      ...
      thus ?thesis using "1.prems"(1) Var
        by (auto simp: vardecl_add_local_def split: sum.splits if_splits)
    next
      case init
...
lemma finalize_match_stmt_facts:
  ...
  shows "coreStmt
    ...
    and "ghost = NotGhost \<Longrightarrow> mode = Var \<Longrightarrow> is_complete_type scrutTy"
    and "\<And>dp vr n ty. dp \<in> set dps \<Longrightarrow> (vr, n, ty) \<in> set (dec_pattern_var_bindings dp)
           \<Longrightarrow> ghost = NotGhost \<Longrightarrow> vr = Var \<Longrightarrow> is_complete_type ty"
  using assms unfolding finalize_match_stmt_def
  by (auto split: if_splits simp: list_ex_iff case_prod_unfold) (fastforce+)
...
next
  ...
  have proj_cp: "ghost = NotGhost \<Longrightarrow> vr = Var \<Longrightarrow> is_complete_type vTy"
    using Cons.prems(12) q_eq by auto
  ...
  proof (cases vr)
    case Var
    thus ?thesis using proj_typed proj_wk proj_rt proj_cp
  ...
  have IH: "core_statement_list_type ?env1 ghost
    ...
    using Cons.IH[OF tail_typed tail_wk tail_rt tail_fv tail_lv tail_wr
                     base_wr1 tail_nobase tail_dist tail_base base_gh1 tail_cp] .
...
lemma wrap_vardecls_types:
  assumes compat: "dec_pattern_compatible env dp scrutTy"
      ...
      and bind_cp: "\<And>vr n vTy. (vr, n, vTy) \<in> set (dec_pattern_var_bindings dp)
                      \<Longrightarrow> ghost = NotGhost \<Longrightarrow> vr = Var \<Longrightarrow> is_complete_type vTy"
...
proof -
  ...
  have projs_cp:
    "\<And>vr n vTy proj. (vr, n, vTy, proj) \<in> set ?projs
       \<Longrightarrow> ghost = NotGhost \<Longrightarrow> vr = Var \<Longrightarrow> is_complete_type vTy"
    using triple_in bind_cp by blast
  ...
  have chain:
    ...
    using vardecl_chain_types[OF projs_typed projs_wk projs_rt projs_fv projs_lv
                                 ...
                                 projs_base ghost_base projs_cp] .
...
next
  ...
  next
    ...
    proof (cases "is_impure_call env elabEnv tm")
      case True
      \<comment> \<open>Impure call: delegated to elab_return_impure_correct.\<close>
      from "7.prems"(1) gh False where_some True have
        eri: "elab_return_impure env elabEnv ghost loc tm next_mv
                = Inr (coreStmt, env', next_mv')"
        by simp
