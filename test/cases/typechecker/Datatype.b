...
interface {}
  ...
  function bar<a>(m: Maybe<a>)
  {
    ...
    var m6 = Nothing;          // error, wrong num tyargs
  }
  ...
  function call_bar()
  {
    ...
    bar<i32>(let n = Nothing in n);        // error, wrong num tyargs
    bar<i32>(let n = Nothing<i32> in n);   // ok
    bar<i32>(let n = Nothing<u32> in n);   // error, type mismatch

    bar<Maybe>(100);           // error, wrong num tyargs
    bar(Nothing<i32>);         // error, wrong num tyargs (tyargs are not inferred currently)
    foo<i32,i64>();            // error, wrong num tyargs
  }
  ...
  const c4 = Nothing;                    // error, wrong num tyargs
  ...
  const eq4: bool = call_bar == call_bar;  // error, == doesn't work on funcs
  ...
  function eqtest()
  {
    ...
    assert (call_bar == call_bar);  // error
  }
