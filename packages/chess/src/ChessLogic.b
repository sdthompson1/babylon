...
function legal_knight_move(pos: Position, from: Square, to: Square): bool
    ...
    requires valid_square(to);
{
    var diff_x = i64_abs(from.x - to.x);
    var diff_y = i64_abs(from.y - to.y);
    ...
    requires from != to;
{
    var dx = (to.x - from.x);
    var dy = (to.y - from.y);
    if i64_abs(dx) == i64_abs(dy) {
        return squares_between_are_empty(pos, from, to, i64_sgn(dx), i64_sgn(dy));
    } else {
        return false;
    }
}
...
function legal_rook_move(pos: Position, from: Square, to: Square): bool
    ...
    requires from != to;
{
    var dx = i64_sgn(to.x - from.x);
    var dy = i64_sgn(to.y - from.y);
    if dx == 0 || dy == 0 {
...
function legal_king_move(pos: Position, from: Square, to: Square): bool
    ...
    requires valid_square(to);
{
    var dx = i64_abs(to.x - from.x);
    var dy = i64_abs(to.y - from.y);
    ...
    requires valid_square(to);
{
    return match pos.board[from.x, from.y] {
        case Just {_, King} =>
            i64_abs(to.x - from.x) == 2    // a 2-space king move is castling
        case _ =>
            false
        };
}
    ...
    requires (pos.board[from.x, from.y] == Just{pos.turn, King});
{
    ...
    var modified_to = {x=from.x + i64_sgn(to.x - from.x), y=to.y};
    ...
    ensures valid_position(pos);
{
    ...
    case Just{col, King} =>
        ...
        if i64_abs(to.x - from.x) == 2 {
            var old_rook_x = if to.x > from.x then x_size - 1 else 0;
            var new_rook_x = from.x + i64_sgn(to.x - from.x);
