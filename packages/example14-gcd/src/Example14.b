...
// This example program will compute the greatest common divisor (GCD)
// of two numbers using Euclid's algorithm. We also include proofs that
...
// (We consider only cases where a != 0 and b != I32_MIN.)
    ...
    ensures divides(a, b*x + c*y);
{
    ...
    if (a == -1) {
        ...
        // Now that we have convinced the verifier that there exists kb
...
function gcd(a_input: i32, b_input: i32): i32
    ...
    // We promise that our return value will be positive, and that it
    // will be a divisor of both a and b.
    ensures return > 0;
