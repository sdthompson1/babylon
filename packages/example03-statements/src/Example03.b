...
// In this example we introduce a few of the statements that can be
// used in the language. Perhaps the most interesting is the "while"
// statement, in which we introduce the concept of loop invariants.
    ...
    requires 0 < x < 100;
{
    ...
    // programmer is claiming that a certain condition is always true at a
    // particular point in the program. Asserts will be proved by the verifier.

    // For example, the following will succeed:
    assert 1 + 2 == 3;
    // (The SMT solvers will be asked to prove that 1 + 2 == 3, which
...
function fun4(x: i32)
{
    ...
    // When doing proofs, the "if" condition may be assumed to be true
    ...
    if 0 < x < 100 {
        ...
        // of the code, then x must be less than 100.
        assert 2*x < 200;
        ...
        // This can be proved, because we know that the "if" condition must be
        // false, given that we have reached this part of the code.
        assert x <= 0 || x >= 100;
    }
}
...
function fun5()
{
    ...
    while i < 10
        ...
        // after the loop exits (that is why we say "<= 10" rather
        // than "< 10").
        invariant 0 <= i <= 10;
...

    {
        ...
        // this, for the reasons given above. If we needed to prove that
    ...
    // now, the effect of a "for" loop can be simulated using a "while" loop.
}
...
function fun6(i: i32)
{
    // The next statement we will introduce is "match".
    ...
    case _ =>
        println("i is neither 1, 2 nor 3");
    }
    ...
    // nearly always necessary to include a "case _".)
    ...
    case false =>
        println("i is less than or equal to 10");
    }
    ...
    // When we come to look at datatypes and records (in a future example),
...
function main()
{
    ...
    // that any "ensures" conditions of f are met, but nothing else about what
    // the function does.)
